void plotTrackLenVsEne(){
  ...
  /*
  double eoi = 200;
  int pbin = TrkLenVsEne->GetXaxis()->FindBin(eoi);
  slice1 = TrkLenVsEne->ProjectionY("slice1", pbin, pbin+1, "e1p");
  slice1->SetLineColor(5);
  eoi = 1000;
  pbin = TrkLenVsEne->GetXaxis()->FindBin(eoi);
  slice2 = TrkLenVsEne->ProjectionY("slice2", pbin, pbin+1, "e1p");
  slice2->SetLineColor(6);
  eoi = 10000;
  pbin = TrkLenVsEne->GetXaxis()->FindBin(eoi);
  slice3 = TrkLenVsEne->ProjectionY("slice2", pbin, pbin+1, "e1p");
