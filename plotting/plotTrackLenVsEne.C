void plotTrackLenVsEne(){
	...
	TH2D* TrkLenVsEne = new TH2D("TrkLenVsEne", "Track Length Vs. Energy; Energy MeV; Tracklength/X_{0}", 5000, 0, 100000, 1000, 0, 18);
	TrkLenVsEne->Sumw2();
	...
	slice1 = TrkLenVsEne_norm->ProjectionY("slice1", pbin, pbin+1, "e1p"); //fill in the projection data
	...
	slice2 = TrkLenVsEne_norm->ProjectionY("slice2", pbin, pbin+1, "e1p");
	slice2->SetLineColor(4);
	
	eoi = 10000;
	pbin = TrkLenVsEne_norm->GetXaxis()->FindBin(eoi);
	slice3 = TrkLenVsEne_norm->ProjectionY("slice3", pbin, pbin+1, "e1p");
